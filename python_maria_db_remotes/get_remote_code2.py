...
def search_buttons(ip_address, manufacturer, button_key):
    ...
    conn = mysql.connector.connect(
        host=ip_address,  # Use the provided IP address
        user='root',
        password='root',
        database='remote_ir'
    )
...
if __name__ == "__main__":
    if len(sys.argv) != 4:
        print("Usage: python script_name.py ip_address manufacturer_name button_key")
        sys.exit(1)
    
    ip_address = sys.argv[1]
    manufacturer_name = sys.argv[2]
    button_key = sys.argv[3]
    search_buttons(ip_address, manufacturer_name, button_key)
